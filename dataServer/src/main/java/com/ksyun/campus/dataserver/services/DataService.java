...
public class DataService {
    ...
    public String write(String path, byte[] data) throws Exception {
        ...
        int flag = 0;
        //如果是目录
        if (path.endsWith("duskgaga")) {
            flag = 1;
            path = path.substring(0, path.length() - "duskgaga".length());
            if (path.endsWith("/")) {
        ...
        if (flag == 0) {
            ...
            // 拼接文件路径
            filePath = projectPath + File.separator + "data" + port + File.separator + path;
        ...
        if(flag == 1){
            //是目录就重新设置size和type
            statInfoSingle.setSize(0);
            statInfoSingle.setType(FileType.Directory);
        }
        //设置ReplicaData
